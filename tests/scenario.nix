...
pkgs.testers.runNixOSTest {
  ...
  nodes.installer = {
    ...
    virtualisation = {
      ...
      # Installation reads a large closure, which is prohibitively slow over 9p.
      useNixStoreImage = true;
      # Generated flake commands build small wrappers during the test.
      writableStore = true;
  ...
  testScript = ''
    ...
    with subtest("unlock and connect to installed system"):
        installer.succeed(retry_shell("${unlock} -F none"))
