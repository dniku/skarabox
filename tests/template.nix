...
pkgs.testers.runNixOSTest {
  ...
  testScript = ''
    installer.start()
    installer.succeed("mkdir /tmp/template")
    installer.succeed(
        "cd /tmp/template && printf 'skarabox1234\\n' | gen-initial -v -y -s",
        timeout=300,
    )
    installer.succeed("test -f /tmp/template/flake.nix")
    installer.succeed("test -f /tmp/template/myskarabox/configuration.nix")
    installer.succeed("test -s /tmp/template/myskarabox/secrets.yaml")
    installer.succeed("test -s /tmp/template/sops.key")
    installer.succeed(
        "cd /tmp/template && "
    ...
    installer.fail(
        "grep -R \"I'm empty and in plain text right now\" /tmp/template"
    )
  '';
}
